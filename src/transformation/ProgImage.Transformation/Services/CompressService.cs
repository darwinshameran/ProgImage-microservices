...
namespace ProgImage.Transformation.Services
{
    public class CompressService : ICompressService
    {
        ...
        public CompressService(IStatusService statusService, IProducer producer)
        {
            _statusService = statusService;
            _producer = producer;
        }

        public async Task<TransformationStatusResponse> TransformImage(Guid imageId, int quality)
        {
            Guid statusId = Guid.NewGuid();
            
            TransformationCompressStartEvent @event = new TransformationCompressStartEvent()
            {
                StatusId = statusId,
            ...
            _producer.Push(@event, "progimage.transformation.compress");
        ...
        public async Task<TransformationStatusResponse> TransformImage(IFormFile image, int quality)
        {
            Guid statusId = Guid.NewGuid();
        ...
        public async Task<TransformationStatusResponse> TransformImage(string url, int quality)
        {
            Guid statusId = Guid.NewGuid();
